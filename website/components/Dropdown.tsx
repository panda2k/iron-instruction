...
const Dropdown: NextPage<Props> = (props: Props) => {
    ...
    return (
        <div className="relative inline-block text-left">
            <div>
                <button onClick={handleClick} type="button" className="inline-flex w-full justify-center rounded-md border border-black bg-white px-4 py-2 text-sm font-medium text-black shadow-sm hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true">
                    {props.heading}
                ...
                {props.logoutButton && (
                    <div className="py-1">
                        <form onSubmit={handleLogout}>
                            <button type="submit" className="text-black block w-full px-4 py-2 text-left text-sm" role="menuitem" tabIndex={-1}>Sign out</button>
                        </form>
                    </div>
                )}
            </div>
